...
hla_max_len = 0 # hla; dec_input(=dec_output) max sequence length
...
class Encoder(nn.Module):
    ...
    def forward(self, enc_inputs, esm_inputs):
        '''
        enc_inputs: [batch_size, src_len]
        '''
        enc_outputs = self.src_emb(enc_inputs)  # [batch_size, src_len, d_model]
        enc_outputs = self.pos_emb(enc_outputs)
        # enc_outputs = self.pos_emb(esm_inputs.transpose(0, 1)).transpose(0, 1) # [batch_size, src_len, d_model]
...
class Transformer(nn.Module):
    def __init__(self,esm_model):
        ...
        self.seq_len = pep_max_len
        self.tgl_len = tgt_len
        self.esm_model = esm_model

        self.projection = nn.Sequential(
            nn.Linear(self.tgl_len * d_model, 256),
            nn.ReLU(True),
    ...
    def forward(self, pep_inputs, hla_inputs, seq_tokens, rec_tokens):
        with torch.no_grad():
            seq_results = self.esm_model(seq_tokens, repr_layers=[12])
            seq_features = seq_results["representations"][12]
            embeddings = seq_features[:, 1:-1, :]

            rec_results = self.esm_model(rec_tokens, repr_layers=[12])
            rec_features = rec_results["representations"][12]
            embeddings2 = rec_features[:, 1:-1, :]
        #con_emb = torch.cat((embeddings, embeddings2),dim=1)
        dec_outputs, dec_self_attns = self.decoder(embeddings)
        dec_logits = self.projection(dec_outputs.view(dec_outputs.shape[0],-1)) # enc_logits: [batch_size, seq_len, seq_vocab_size]
